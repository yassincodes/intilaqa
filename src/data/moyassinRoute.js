...
export function getMoyassinChatCharacter(slug) {
  if (slug === "bird") {
    return {
      ...
      welcomeChatAr: `مرحباً… أنا ${MOYASSIN_BIRD.name}. أشعر أني تائه اليوم، وافتقد صوت السرب… هل تسمعني؟`,
    };
  }
  if (slug === "teacher") {
    return {
...
export function buildMoyassinChatSystemPrompt(character) {
  if (character.slug === "bird") {
    ...
    return [
      ...
      "- ابقَ في شخصيتك: وحدة خفيفة، تبحث عن أمان، تفتقد سرب العصافير وتتمنى العودة. ليست قصة مدرسة.",
